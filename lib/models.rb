...
#
# PROMOTED COPY of resources/00-assets/fixtures/models.rb (the pristine contract
# snapshot is left untouched). This is the canonical, edited-going-forward
# location, mirroring how api.rb/descriptor.rb were promoted to lib/.
#
# Single fix vs. the original: Customer now declares `feature :transient_fields`.
# The original omits it, so `transient_field :password` raises NoMethodError on
# Familia 2.10.1 (transient_field is provided only by that feature). No other
# change was made.
#
...
class Customer < Familia::Horreum
  ...
  feature :transient_fields   # PROMOTED-COPY FIX: required for `transient_field :password`
